...
const ProgramPerformance = ({ programId }: { programId: string }) => {
    const { data: analytics, isLoading, error } = useQuery({
        queryKey: ['programAnalytics', programId],
        queryFn: () => ProgramRepository.getProgramAnalytics(programId),
    ...
    if (isLoading) return <div className="p-10 text-center text-slate-400 animate-pulse">Loading analytics...</div>;
    if (error) return <div className="p-10 text-center text-red-400">Failed to load analytics</div>;

    // Safety check for invalid data structure
    if (!analytics || !analytics.exams) {
        return <div className="p-10 text-center text-slate-500">No analytics data available for this program.</div>;
    }
